...
def sgd_optimizer(lr_func, mom: float = 0.9, wd: float = 0.0):
    ...
    def opt_func():
        lr = lr_func()
        my_opt = tf.train.AdamOptimizer(learning_rate=lr)
        # return SGD(lr, mom=mom, wd=wd)
        # my_opt = LAMBOptimizer(learning_rate=lr, weight_decay_rate=wd, beta_1=0.9, beta_2=0.999, epsilon=1e-6, exclude_from_weight_decay=["LayerNorm", "layer_norm", "batch_normalization", "BatchNormalization", "bias"])
        return my_opt
